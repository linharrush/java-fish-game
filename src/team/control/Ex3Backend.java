...
public class Ex3Backend {
    ...
    public void oceanView(){
        ...
        ex3UiPort().updateBackgroundToOceanView();

        Canvas canvas = App.content().canvas();
        for (int i = 0; i < 3; i++) {
            Circle c = canvas.getCircle(i);
            ex3UiPort().updateCircleToOceanView(i, c.getCenter().getX(), c.getCenter().getY(), c.getR());
        }
    }

    public void normalView(){
        this.oceanViewState = false;
        ex3UiPort().updateBackgroundToNormalView();

        Canvas canvas = App.content().canvas();
        for (int i = 0; i < 3; i++) {
            Circle c = canvas.getCircle(i);
            ex3UiPort().backCircleToNormalView(i, c.getCenter().getX(), c.getCenter().getY(), c.getR());
        }
    }
